...
class nginxParser():
    ...
    def error_analysis(self, grp, isoFormat, dt_ts):
        ...
        if len(self.error_dqs[grp['ip']]) >= error_threshold:
            self.sus_ips_error_per_min.append({"dbTimestamp": isoFormat,"IP" : grp['ip'], "Type": ip_type(grp['ip']), "from" : self.error_dqs[grp['ip']][0].isoformat(), "to" : self.error_dqs[grp['ip']][-1].isoformat(),"No. of errors in a min" : len(self.error_dqs[grp['ip']])})
    ...
    def rt_analysis(self, grp, isoFormat):
        if grp['status_code'] == '499':
            self.incomplete_responses.append({"dbTimestamp" : isoFormat , "IP" : grp['ip'], "Type": ip_type(grp['ip']), "Timestamp" : grp['timestamp'],"Status Code" : grp['status_code'],"Response Time" : grp['rt'] })
