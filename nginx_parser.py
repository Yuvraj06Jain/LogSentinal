...
def rt_analysis(ip_details:dict):
    response_incomplete = defaultdict(list)

    for ip in ip_details:
        for grp in ip_details[ip]:
            if(grp['status_code'] == '499'):
                response_incomplete[ip].append(grp)

    print("Responses that were closed before they could be finished")
    for ip in response_incomplete.keys():
        for x in response_incomplete[ip]:
            print(f"{ip} || {grp['status_code']} || {x['rt']} || {x['timestamp']}")
    print()
